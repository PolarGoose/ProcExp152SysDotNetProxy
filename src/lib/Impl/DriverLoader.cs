...
internal static class DriverLoader_CreateOrOpenService
{
    ...
    public static SafeServiceHandle CreateOrOpenService(SafeServiceHandle serviceManager, string driverServiceName, string driverFileFullName)
    {
        ...
        {
            throw new ProcExp152SysDotNetProxyWinApiException("CreateService", "Failed to create the driver service");
        }

        service = OpenService(serviceManager, driverServiceName, SERVICE_ALL_ACCESS);
        if (service.IsInvalid)
        {
            throw new ProcExp152SysDotNetProxyWinApiException("OpenService", "Failed to open existing driver service");
        }
...
internal static class DriverLoader_LoadDriver
{
    ...
    public static void LoadDriver(string driverServiceName, string driverFileFullName)
    {
        ...
        {
            throw new ProcExp152SysDotNetProxyWinApiException("OpenSCManager", "Failed to open Service Manager");
        }
        ...
        {
            throw new ProcExp152SysDotNetProxyWinApiException("StartService", "Failed to start the service");
        }
    }
}
...
internal static class DriverLoader
{
    ...
    private static void EnsureRunAsAdmin()
    {
        ...
        {
            throw new ProcExp152SysDotNetProxyException("The process needs to be run from an admin");
        }
    }
