    ...
    public static SYSTEM_HANDLE_TABLE_ENTRY_INFO_EX[] QuerySystemHandleInformation()
    {
        for (var buffSize = 32 * 1024 * 1024 /* 32Mb */; buffSize <= 1024 * 1024 * 1024 /* 1Gb */; buffSize *= 2)
        {
            ...
            try
            {
                ...
                {
                    throw new ProcExp152SysDotNetProxyException($"NtQuerySystemInformation failed with status {status}");
                }
            }
            finally
            {
        ...
        throw new ProcExp152SysDotNetProxyException("NtQuerySystemInformation buffer size is not enough");
    }
